...
string get_table_data(char mnemonic[10], int flag);
void get_table_number(table obj);
...
int get_literal_address(int lt);
int get_symbol_address(char op2[], symbol_table sym_table[50]);
int generate_target_code(char label[10], char mnemonic[10], char opcode[8], char op2[5], symbol_table sym_table[50]);
...
int main(){
	...
	lc = atoi(c.op2); // starting location
	//Insertion Format 
	...
	while(--lines){
		...
		/*if (strcmp(c.mnemonic, "LTORG") == 0){
			lc++;
			ltorg_array[ltorg_count-1] = lc;
		}*/
		if(strcmp(c.mnemonic, "END") == 0)
			break;
		else{
			char c1 = c.op2[0]; 
			lc++; 
	...
	//Read from the File
	ifstream in2("pass2.txt");
	cout << "\n TARGET CODE: " << endl;
	while (in2 >> c.label >> c.mnemonic >> c.opcode >> c.op2){
		if(generate_target_code(c.label, c.mnemonic, c.opcode, c.op2, sym_table) ==1)
			continue;
		else{
			cout << " \n EXITTING " << endl; 
			break;
...
int code_generation(char label[10], char mnemonic[10], char opcode[8], char op2[5]){
	...
	if( (strcmp(label, "-") == 0 ) && (strcmp(mnemonic, "START") == 0) ){
		...
		cout << add_count; 
		if(get_table_data(mnemonic, flag) == " ") {
...
int address_count=0, lt=0, lit=0, st=0; 
int generate_target_code(char label[10], char mnemonic[10], char opcode[8], char op2[5], symbol_table sym_table[50]){
	int flag = 0;
	ln_cnt++; 
	if( (strcmp(label, "-") == 0 ) && (strcmp(mnemonic, "START") == 0) ){
		address_count = atoi(op2); 
		cout << " 01 \t - \t - \t " << address_count <<  endl; 
	}
		...
		if(get_table_data(mnemonic, flag) == " ") {
			...
			if(strcmp(mnemonic, "DC") == 0 || strcmp(mnemonic, "DS") == 0)
				cout << "\t -  " << endl;
			else{
				char c = op2[0]; 
				if(c == '='){
					cout << "\t"<< get_literal_address(lt);
					lt++;
				}	 
				if(strcmp(mnemonic, "DC") == 0 || strcmp(mnemonic, "DS") == 0)
					cout << "\t - \t - " << endl;
				else if(c != '=' && c != '-'){				
					cout << "\t"<< get_symbol_address(op2, sym_table);
					cout << endl;
				}
				else 
					cout << endl;
...
string get_table_data(char mnemonic[10], int flag){
	...
	if(flag == 1){
		...
		if(str == "START" || str == "END" || str == "LTORG" || str == "ORG" || str == "EQU")
			get_table_number(pseudo_opcode.find(str)->second);
		else if(str == "DC" || str == "DS")
			get_table_number(declaration.find(str)->second);
		else if(str == "STOP" || str == "ADD" || str == "SUB" || str == "MULT" || str == "MOVER" || str == "MOVEM" || str == "COMP" || str == "BC" || str == "DIV" || str == "READ" || str == "PRINT")
			get_table_number(mnemonic_opcode.find(str)->second);	
		else {
			cout << "\n----------ERROR AT LINE NO: " << line_count << "(invalid Mnemonic) ---------" << endl;
			return "....EXITTING....";
		}
...
int get_literal_address(int lt){
	return ltorg_array[lt];
}

int get_symbol_address(char op2[5] , symbol_table sym_table[50]){
	for(int i=0; i<50; i++){
		if(strcmp(op2, sym_table[i].symbol) == 0)
